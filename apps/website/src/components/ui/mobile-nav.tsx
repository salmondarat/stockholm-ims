...
export default function MobileNav() {
  ...
  return (
    <div className="md:hidden relative">
      <button
        className="btn btn-pill text-sm border-gray-300 text-gray-900"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
      ...
      {open && (
        <div
          id="mobile-nav-panel"
          className="absolute right-0 mt-2 w-72 rounded-2xl border bg-white text-gray-900 p-2 shadow-2xl"
        >
          ...
          <div className="mt-2 border-t border-gray-200 pt-2 grid gap-2">
            ...
            <Link
              href={`${appUrl}/signup`}
              className="btn btn-cta btn-pill text-sm"
              onClick={() => setOpen(false)}
            >
              Start Free Trial
            </Link>
...
function NavLink({
  ...
  return (
    <Link
      href={href}
      onClick={onClick}
      className="block rounded-md px-3 py-2 text-sm hover:bg-gray-50 hover:text-[--primary] transition-colors"
    >
      {children}
    </Link>
  );
...
function Collapsible({
  ...
  return (
    <div className="rounded-md">
      <button
        className="group w-full flex items-center justify-between rounded-md px-3 py-2 text-sm hover:bg-gray-50 hover:text-[--primary] transition-colors"
        onClick={() => setOpen((v) => !v)}
      >
        <span>{label}</span>
        <ChevronDown
          className={`h-4 w-4 transition-transform duration-200 ${open ? "rotate-180" : ""} group-hover:-translate-y-0.5 group-hover:rotate-180`}
        />
      </button>
...
function MegaLink({
  ...
  return (
    <Link
      href={href}
      className="group flex items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-gray-50 transition-colors"
    >
