  ...
  return (
    ...
    <div ref={rootRef} className={inline ? "block ml-0" : "relative mt-1"}>
      <button
        ...
        className={
          inline
            ? "text-[11px] underline text-gray-600 align-baseline"
            : "text-[11px] underline text-gray-600"
        }
        onClick={() => setOpen((v) => !v)}
      >
        ...
        open && (
          <div className="mt-1 w-full rounded-md border bg-gray-50 dark:bg-neutral-900/40 p-2 space-y-1 break-words">
            {list.map((v, i) => (
              <div
                key={i}
                className="text-[11px] text-gray-600 flex items-center justify-between"
              >
                <div className="flex flex-wrap gap-1">
                  {Object.entries(v.attrs).map(([k, val]) => (
                    <span
                      key={k + val}
                      className="px-2 py-0.5 rounded-full border break-words"
                    >
                      {k}: {val}
                    </span>
                  ))}
